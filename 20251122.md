...
JAPOINTシステムは、6つのスマートコントラクトで構成されています：
...
| **Transfer10** | 決済分配（1%手数料） | - 決済の受付<br>- 1%/99%分配<br>- ポイント付与依頼 | JPYD, JAPointMint |
| **Transfer5** | 決済分配（0.5%手数料） | - 決済の受付<br>- 0.5%/99.5%分配<br>- ポイント付与依頼 | JPYD, JAPointMint |
| **JAPointMint** | ポイント発行管理 | - JPYD受取<br>- JAPT Mint<br>- 会社への手数料送付 | JPYD, JAPoint |
| **JAPoint** | ポイントトークン | - ERC20標準機能<br>- Mint（認可制） | なし |
...
Transfer10._processPayment(amount, User)
  ...
  │       ├─→ JAPoint.mint(User, fee * 2)  // 1JPYD → 2JAPT
