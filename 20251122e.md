...
| **Transfer10** | Payment Distribution (1% fee) | - Payment Reception<br>- 1%/99% Distribution<br>- Point Award Request | JPYD, JAPointMint |
| **Transfer5** | Payment Distribution (0.5% fee) | - Payment Reception<br>- 0.5%/99.5% Distribution<br>- Point Award Request | JPYD, JAPointMint |
| **JAPointMint** | Point Issuance Management | - JPYD Reception<br>- JAPT Minting<br>- Fee Transfer to Company | JPYD, JAPoint |
| **JAPoint** | Point Token | - ERC20 Standard<br>- Mint (Authorized only) | None |
...
Transfer10._processPayment(amount, User)
  ...
  │
  ├─→ [JAPT Award Flow]
  │   ├─→ JPYD.approve(JAPointMint, fee)
  │   └─→ JAPointMint.transferJAPoint(User)
  │       ├─→ JPYD.transferFrom(Transfer10, JAPointMint, fee)
  │       ├─→ JAPoint.mint(User, fee * 2)  // 1 JPYD → 2 JAPT
...
// JAPointMint
event JAPointTransferred(
    address indexed user,
    uint256 jpydAmount,
    uint256 japointAmount
);
```
...
contract Transfer10 is ITokenReceiver {
    ...
    function _processPayment(uint256 amount, address sender) internal {
        // 1% fee calculation
        uint256 japointMintAmount = amount / 100;      // 1% → for JAPT award
        uint256 shopAmount = amount - japointMintAmount; // 99% → to shop

        // 1. Approve JAPointMint & request point award
...
2. Enter amount
3. Approve signature
4. **JAPT points instantly deposited to wallet**
...
// Complete! JAPT automatically awarded to user's wallet
```
...
| JAPoint (JAPT) | `0x2eDf302548B23e9F599e483aE79cda6D8774c6fC` | [View](https://sepolia.etherscan.io/address/0x2eDf302548B23e9F599e483aE79cda6D8774c6fC) |
...
- JAPoint mint permission limited to JAPointMint contract only
